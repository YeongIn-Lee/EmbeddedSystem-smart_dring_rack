...
static void initMeasureTimer() {
	...
	TIM_OC4Init(US_TIMER, &TIM_OCInitStruct);
	...
	TIM_ClearFlag(US_TIMER, TIM_FLAG_Update);
        ...
        //TIM2
        TIM_DeInit(US_TIMER2);
	TIM_TimeBaseInitStruct.TIM_Prescaler = prescaler;
	TIM_TimeBaseInitStruct.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseInitStruct.TIM_Period = 0xFFFF;
	TIM_TimeBaseInitStruct.TIM_ClockDivision = TIM_CKD_DIV1;
	TIM_TimeBaseInit(US_TIMER2, &TIM_TimeBaseInitStruct);
	...
	TIM_OCInitStruct.TIM_Pulse = 15; //us
	TIM_OCInitStruct.TIM_OCPolarity = TIM_OCPolarity_High;
	TIM_OC3Init(US_TIMER2, &TIM_OCInitStruct);
	...
	TIM_PWMIConfig(US_TIMER2, &TIM_ICInitStruct);
	TIM_SelectInputTrigger(US_TIMER2, US_TIMER_TRIG_SOURCE);
	TIM_SelectMasterSlaveMode(US_TIMER2, TIM_MasterSlaveMode_Enable);

	TIM_CtrlPWMOutputs(US_TIMER2, ENABLE);

	TIM_ClearFlag(US_TIMER2, TIM_FLAG_Update);
}
...
static void initPins() {
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(US_ECHO_PORT2, &GPIO_InitStructure);
}
...
int32_t HCSR04GetDistance2() {
	(US_TIMER2)->CNT = 0;
	TIM_Cmd(US_TIMER2, ENABLE);
	while(!TIM_GetFlagStatus(US_TIMER2, TIM_FLAG_Update));
	TIM_Cmd(US_TIMER2, DISABLE);
	TIM_ClearFlag(US_TIMER2, TIM_FLAG_Update);
	return (TIM_GetCapture2(US_TIMER2)-TIM_GetCapture1(US_TIMER2))*165/1000;
}